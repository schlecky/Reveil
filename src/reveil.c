...
int timerSnooze, volMax, volMin, numSonnerie, cptSnooze;
int numAlarme=0;
int blMax, blMin;
int flashCnt;
...
{        
    if(P2IFG & (BTN_MENU | BTN_HAUT| BTN_BAS))
    {
      ...
      blTimer=BL_DURATION; //5 secondes
    }
    if(P2IFG & BTN_MENU)
    {
        btnAppuye = MENU;
...
interrupt (WDT_VECTOR) Watchdog(void)
{
  ...
  if(WDTcnt==64)
  {
    ...
    if(date.heure.secondes==60)
    {
      ...
      if(date.heure.heures == 3)
      {
        // 3h18 ?
	if(date.heure.minutes == 18)
        {
          config |= FLASH_LIGHT; 
        }
        else if(date.heure.minutes == 0)
        {
          DCF_POW_ON;
        }
      }
